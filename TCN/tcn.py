...
def is_power_of_two(num: int):            #判断d是否为2
    return num != 0 and ((num & (num - 1)) == 0)


def adjust_dilations(dilations: list):      #若不满足，则进行修改
...
class ResidualBlock(Layer):
    ...
    def __init__(self,
                 dilation_rate: int,       #d值
                 nb_filters: int,          #卷积核个数
                 kernel_size: int,         #卷积核大小
                 padding: str,             #填充
                 activation: str = 'relu',   #激活函数
                 dropout_rate: float = 0,       #dropout率
                 kernel_initializer: str = 'he_normal',       #卷积核权重初始化
        ...
        Args:
            ...
            padding: The padding used in the convolutional layers, 'same' or 'causal'.（因果填充，只填充一边）
        ...
        self.layers = []
        self.layers_outputs = []
        self.shape_match_conv = True
        self.res_output_shape = True
        ...
        with K.name_scope(self.name):  # name scope used to make sure weights get unique names
            ...
            for k in range(2):                #构建两个卷积层
                ...
                with K.name_scope(name):  # name scope used to make sure weights get unique names
                    ...
                    if self.use_weight_norm:                #权重标准化
                        from tensorflow_addons.layers import WeightNormalization     #此处安装了一个addons库
                ...
                with K.name_scope('norm_{}'.format(k)):
                    if self.use_batch_norm:                            #批次标准化
                        self._build_layer(BatchNormalization())
                    elif self.use_layer_norm:                          #通道标准化
                        self._build_layer(LayerNormalization())
                    elif self.use_weight_norm:
                        pass  # done above.

                self._build_layer(Activation(self.activation))         #激活函数
                self._build_layer(SpatialDropout1D(rate=self.dropout_rate))   #dropout率

            if self.nb_filters != input_shape[-1]:                  #通道数匹配
                # 1x1 conv to match the shapes (channel dimension).
                name = 'matching_conv1D'
                with K.name_scope(name):
                    # make and build this layer separately because it directly uses input_shape
                    self.shape_match_conv = Conv1D(filters=self.nb_filters,         #构建1*1卷积
                                                   kernel_size=1,
                                                   padding='same',
                                                   name=name,
                                                   kernel_initializer=self.kernel_initializer)
    ...
    def call(self, inputs, training=None):          #进行一个跳跃连接
        """
        ...
        for layer in self.layers:                   #进行一个特征融合
            training_flag = 'training' in dict(inspect.signature(layer.call).parameters)   #将参数信息用字典返回
...
class TCN(Layer):
    """Creates a TCN layer.
        ...
        Args:
            ...
            nb_stacks : The number of stacks of residual blocks to use.（堆积了多少个块）
            padding: The padding to use in the convolutional layers, 'causal' or 'same'.
            use_skip_connections: Boolean. If we want to add skip connections from input to each residual blocK.（利用布尔值确定是否要用跳跃连接）
    ...
    def __init__(self,
                 nb_filters=32,      #64个卷积核
                 kernel_size=2,      #2*2的卷积核
                 nb_stacks=1,        #只用了一个块
                 dilations=(1, 2, 4),     #d值
                 padding='causal',                   #因果填充，只填一边
                 use_skip_connections=False,         #跳跃连接
                 dropout_rate=0.2,
                 return_sequences=False,             #只返回最后一个输出序列
                 activation='relu',
                 kernel_initializer='he_normal',     #权重使用He初始化方法
        ...
        if self.use_batch_norm + self.use_layer_norm + self.use_weight_norm > 1:   #只能规定一个规范化操作
            raise ValueError('Only one normalization can be specified at once.')

        if isinstance(self.nb_filters, list):           #保证层数一致
    ...
    @property
    def receptive_field(self):     #计算卷积的感受野
    ...
    def build(self, input_shape):
        ...
        for s in range(self.nb_stacks):
            for i, d in enumerate(self.dilations):   #i为顺序数，d为膨胀率
                res_block_filters = self.nb_filters[i] if isinstance(self.nb_filters, list) else self.nb_filters
                self.residual_blocks.append(ResidualBlock(dilation_rate=d,                            #构建卷积层，层数为6
...
def compiled_tcn(num_feat,  # type: int       #特征数
                 num_classes,  # type: int    #分类数
                 nb_filters,  # type: int
                 kernel_size,  # type: int
                 dilations,  # type: List[int]
                 nb_stacks,  # type: int
                 max_len,  # type: int        #最大的序列长度数
